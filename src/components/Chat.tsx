...
const Chat: React.FC<ChatProps> = ({ currentUserId, currentUserName, selectedUser, onBack }) => {
  ...
  return (
    <div className="relative w-full h-full flex flex-col bg-white/10 backdrop-blur-lg rounded-2xl p-4 shadow-2xl border border-white/20">
      ...
      <div className="flex flex-col items-stretch space-y-2 mt-2 bg-slate-900/60 rounded-xl p-2 border border-green-600 relative">
        {/* Voice controls panel */}
        {showVoiceControls && (
          <div className="w-full flex items-center space-x-2">
            <button
              onClick={recording ? handleStopRecording : handleStartRecording}
              className={`p-3 rounded-xl text-white ${recording ? 'bg-red-600' : 'bg-blue-600'} hover:opacity-90 transition-colors`}
            >
              {recording ? 'Arrêter' : 'Enregistrer'}
            </button>
            <button
              onClick={handleSendAudio}
              disabled={audioChunks.length === 0 || recording}
              className="p-3 bg-green-600 rounded-xl text-white hover:bg-green-500 transition-colors"
            >
              Envoyer audio
            </button>
            <span className="text-red-400 font-bold">{recordTime}s</span>
            <button onClick={() => setShowVoiceControls(false)} className="ml-auto text-sm text-slate-300 hover:underline">Fermer</button>
          </div>
        )}
        ...
        <div className="flex items-center gap-2">
          <div className="relative">
            <button 
              ...
              className="p-2.5 bg-slate-700 rounded-xl text-white hover:bg-slate-600 transition-colors" 
            ...
            {showCameraOptions && (
              <div className="absolute bottom-full mb-2 left-0 flex flex-col space-y-2 bg-slate-800 p-2 rounded-lg shadow-xl border border-white/20">
                ...
                <button
                  onClick={() => { setShowVoiceControls(true); setShowCameraOptions(false); }}
                  className="p-3 bg-indigo-600 hover:bg-indigo-700 rounded-lg text-white transition-colors text-sm font-medium w-full text-left flex items-center space-x-2"
                  title="Message vocal"
                >
          ...
          <input 
            ...
            placeholder="Message..." 
            className="flex-1 min-w-0 px-3 py-2 border border-slate-600 rounded-xl bg-slate-800/50 text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-green-500" 
            disabled={recording || uploadingMedia} 
          />
          
          <button 
            onClick={audioChunks.length > 0 && !recording ? handleSendAudio : handleSendMessage} 
            className="p-2.5 bg-green-600 rounded-xl text-white hover:bg-green-500 border-2 border-green-400 disabled:opacity-50 transition-colors" 
            disabled={uploadingMedia || recording || (audioChunks.length === 0 && !newMessage.trim())}
          >
